...
export default function Tag({ tag, tagColor }: { tag: string | null, tagColor: string}) {
    ...
    return (
        <div className={"relative w-fit h-fit"}>
            <button onClick={toggleActive} style={{backgroundColor: tagColor}} className={`w-fit text tracking-wide text-gray-100 px-[10px] py-[3px] rounded-xl shadow hover:shadow-md smoothe ${dosis.light}`}>{tag || "Choose Tag"}</button>
